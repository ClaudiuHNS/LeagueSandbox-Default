...
namespace Spells
{
    public class KatarinaR : IGameScript
    {
        ...
        public void OnStartCasting(IChampion owner, ISpell spell, IAttackableUnit target)
        {
            ...
            {
                if (unit.Team != owner.Team && !(unit.IsDead) && unit is ObjAiBase && !(unit is BaseTurret))
                {
            ...
            if (hasbuff == true)
            {
                ((ObjAiBase)target).AddBuffGameScript("KatarinaRTriggerSound", "KatarinaRTriggerSound", spell, 2.5f, true);
