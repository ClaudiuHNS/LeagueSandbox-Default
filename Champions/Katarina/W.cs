...
namespace Spells
{
    public class KatarinaW : IGameScript
    {
        ...
        public void OnFinishCasting(IChampion owner, ISpell spell, IAttackableUnit target)
        {
            ...
            {
                if ((unit.Team != owner.Team) && unit is IObjAiBase && !(unit is IBaseTurret))
                {
                    ...
                    hasbuff = owner.HasBuffGameScriptActive("KatarinaWHaste", "KatarinaWHaste");
                    //LogInfo("HasBuff: " + hasbuff);

                    if ((unit is IChampion) && !hasbuff)
                    {
                        hasbuff = true;
                    }
                    else
            ...
            if (hasbuff == true)
            {
                owner.AddBuffGameScript("KatarinaWHaste", "KatarinaWHaste", spell, 1.0f, true);
