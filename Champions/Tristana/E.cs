...
namespace Spells
{
    public class DetonatingShot : IGameScript
    {
        ...
        {
            var ap = _championRef.Stats.AbilityPower.Total * 0.25f;
            var damage = 25f + _spell.Level * 25f + ap;
            if (target.IsDead == true)
            {
                foreach (var enemyTarget in GetUnitsInRange(target, 300, true)
                    .Where(x => x.Team == CustomConvert.GetEnemyTeam(_championRef.Team)))
                {                    
                    enemyTarget.TakeDamage(_championRef, damage, DamageType.DAMAGE_TYPE_MAGICAL, DamageSource.DAMAGE_SOURCE_SPELL, false);                    
                }
            }
